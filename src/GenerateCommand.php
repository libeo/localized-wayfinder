...
{
    protected $signature = 'wayfinder:generate-localized {--path=} {--skip-actions} {--skip-routes} {--with-form}';
    ...
    public function handle()
    {
        ...
        if (!$this->option('skip-routes')) {
            ...
            $named = $routes->filter(fn(Route $route) => $route->name())->groupBy(function (Route $route) {
    ...
    private function writeNamedFile(Collection $routes, string $namespace): void
    {
        ...
        $localizedRoutes = $routes->filter(fn(Route $route) => $route->isLocalized());
        $nonLocalizedRoutes = $routes->reject(fn(Route $route) => $route->isLocalized());

        $nonLocalizedRoutes->each(fn(Route $route) => $this->writeNamedMethodExport($route, $path));

        if ($localizedRoutes->isNotEmpty()) {
            $pathKey = '@inertiajs/vue3';
            $this->imports[$path][$pathKey] = [
                ...($this->imports[$path][$pathKey] ?? []),
                ...['usePage'],
            ];
            $localizedRoutes->groupBy(function (Route $route) {
                $nameParts = explode('.', $route->name());
                array_shift($nameParts);

                return implode('.', $nameParts);
            })->each(function (Collection $grouped) use ($path) {

                $this->writeNamedLocalizedMethodExport($grouped, $path);
            });
        }
    }
    ...
    private function writeNamedLocalizedMethodExport(Collection $routes, string $path): void
    {
        $route = $routes->first();
        $this->appendContent($path, $this->view->make('wayfinder::localized-method', [
            'controller' => $route->controller(),
            'method' => $route->namedMethod(),
            'original_method' => $route->originalJsMethod(),
            'isInvokable' => $route->hasInvokableController(),
            ...
            'parameters' => $route->parameters(),
            'verbs' => $route->verbs(),
            'uris' => $routes->mapWithKeys(fn(Route $r) => [$r->getRouteLocale() => $r->uri()]),
            'withForm' => $this->option('with-form') ?? false,
        ]));
    }
